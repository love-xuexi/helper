...
class SuperBizAgentApp {
    constructor() {
        ...
        this.lastQuestion = ''; // 最近一次用户提问（反馈关联用）
        this.feedbackTags = ['回答不准确', '文档已过时', '逻辑不清晰', '未解决实际问题', '引用来源错误', '其他'];
        ...
        try {
            ...
            if (data.code === 200 || data.message === 'success') {
                ...
                if (chatResponse && chatResponse.success) {
                    ...
                    this.attachAssistantExtras(messageElement, {
        ...
        try {
            ...
            try {
                while (true) {
                    ...
                    if (done) {
                        // 流结束，使用统一的处理方法
                        this.handleStreamComplete(assistantMessageElement, fullResponse, streamSources);
                        break;
                    }
                        ...
                        if (line.startsWith('id:')) {
                            ...
                            try {
                                ...
                                if (sseMessage && typeof sseMessage.type === 'string') {
                                    ...
                                    } else if (sseMessage.type === 'sources') {
                                        // 命中的引用来源片段，在回答完成后展示
                                        streamSources = Array.isArray(sseMessage.data) ? sseMessage.data : [];
    ...
    // 处理流式传输完成
    handleStreamComplete(assistantMessageElement, fullResponse, sources = []) {
        if (assistantMessageElement) {
            ...
            this.attachAssistantExtras(assistantMessageElement, {
                question: this.lastQuestion,
                answer: fullResponse,
                sources: sources
            });
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (Array.isArray(data.data) && data.data.length > 0) {
                    this.feedbackTags = data.data;
                }
            }
        } catch (e) {
            console.warn('加载反馈标签失败，使用默认标签:', e);
        }
    }

    // 为助手消息附加引用来源展示和反馈按钮
    attachAssistantExtras(messageElement, { question, answer, sources }) {
        if (!messageElement) return;
        const wrapper = messageElement.querySelector('.message-content-wrapper');
        if (!wrapper) return;

        const chunkIds = (sources || []).map(s => s.id).filter(Boolean);

        // 引用来源（可折叠，参考 ChatGPT/Claude 的引用卡片）
        if (sources && sources.length > 0) {
            const sourcesDiv = document.createElement('div');
            sourcesDiv.className = 'message-sources';
            const toggle = document.createElement('button');
            toggle.className = 'sources-toggle';
            toggle.innerHTML = `引用来源 (${sources.length}) <span class="sources-arrow">▾</span>`;
            ...
            sources.forEach((source, index) => {
                ...
                const name = source.document_name || '未知文档';
                const similarity = typeof source.similarity === 'number' ? (source.similarity * 100).toFixed(0) + '%' : '';
                const content = (source.content || '').substring(0, 200);
                item.innerHTML = `
                    <div class="source-item-header">
                        <span class="source-index">[${index + 1}]</span>
                        <span class="source-doc-name">${this.escapeHtml(name)}</span>
                        ${similarity ? `<span class="source-similarity">相关度 ${similarity}</span>` : ''}
                    </div>
                    <div class="source-item-content">${this.escapeHtml(content)}${(source.content || '').length > 200 ? '...' : ''}</div>
                `;
            ...
            toggle.addEventListener('click', () => {
                ...
                toggle.querySelector('.sources-arrow').textContent = isHidden ? '▴' : '▾';
            });
        ...
        likeBtn.title = '点赞';
        likeBtn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7 10v12M15 5.88L14 10h5.83a2 2 0 0 1 1.92 2.56l-2.33 8A2 2 0 0 1 17.5 22H4a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2.76a2 2 0 0 0 1.79-1.11L12 2a3.13 3.13 0 0 1 3 3.88z" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
        const dislikeBtn = document.createElement('button');
        dislikeBtn.className = 'feedback-btn dislike-btn';
        dislikeBtn.title = '不喜欢';
        dislikeBtn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M17 14V2M9 18.12L10 14H4.17a2 2 0 0 1-1.92-2.56l2.33-8A2 2 0 0 1 6.5 2H20a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-2.76a2 2 0 0 0-1.79 1.11L12 22a3.13 3.13 0 0 1-3-3.88z" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>`;

        const setSubmitted = (activeBtn) => {
        ...
        likeBtn.addEventListener('click', async () => {
            const ok = await this.submitFeedback({ question, answer, rating: 'like', chunkIds });
            if (ok) {
                setSubmitted(likeBtn);
                this.showNotification('感谢您的反馈！', 'success');
            }
        });
        dislikeBtn.addEventListener('click', () => {
            this.openFeedbackModal({ question, answer, chunkIds }, () => setSubmitted(dislikeBtn));
        });
    ...
    // 提交反馈到后端
    async submitFeedback({ question, answer, rating, tags = [], comment = '', chunkIds = [] }) {
        try {
            ...
            this.showNotification('提交反馈失败: ' + e.message, 'error');
            return false;
        }
    }

    // 打开结构化负反馈弹窗（不喜欢）
    openFeedbackModal({ question, answer, chunkIds }, onSubmitted) {
        // 避免重复弹窗
        ...
        modal.innerHTML = `
            <div class="feedback-modal-header">
                <span>告诉我们哪里不够好</span>
                <button class="feedback-modal-close" title="关闭">×</button>
            </div>
            <div class="feedback-tags"></div>
            <textarea class="feedback-comment" placeholder="补充描述（选填）：具体哪里不准确、期望的回答是什么..." rows="3"></textarea>
            <div class="feedback-modal-footer">
                <button class="feedback-cancel-btn">取消</button>
                <button class="feedback-submit-btn">提交反馈</button>
            </div>
        `;

        const tagsContainer = modal.querySelector('.feedback-tags');
        ...
        modal.querySelector('.feedback-modal-close').addEventListener('click', close);
        modal.querySelector('.feedback-cancel-btn').addEventListener('click', close);
        overlay.addEventListener('click', (e) => {
            if (e.target === overlay) close();
        });

        modal.querySelector('.feedback-submit-btn').addEventListener('click', async () => {
            const comment = modal.querySelector('.feedback-comment').value.trim();
            if (selectedTags.size === 0 && !comment) {
                this.showNotification('请至少选择一个标签或填写补充描述', 'warning');
                return;
            }
            ...
            if (ok) {
                ...
                this.showNotification('感谢您的反馈，我们会持续改进！', 'success');
            }
        });
    ...
    handleFileSelect(event) {
        ...
        if (file) {
            ...
            if (!this.validateFileType(file)) {
                this.showNotification('不支持的文件格式，支持文档/表格/图片等常见格式', 'error');
    ...
    // 验证文件类型（知识库平台支持多源异构解析）
    validateFileType(file) {
        const fileName = file.name.toLowerCase();
        const allowedExtensions = ['.txt', '.md', '.markdown', '.pdf', '.doc', '.docx', '.xls', '.xlsx', '.csv', '.ppt', '.pptx', '.png', '.jpg', '.jpeg'];
